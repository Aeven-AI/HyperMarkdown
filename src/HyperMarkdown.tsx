...
import MarkdownStream from "./core/MarkdownStream";
 ...
 *
 * The work happens in {@link MarkdownStream}, which is plain TypeScript and
 * holds no React state. This component owns one instance of it, subscribes to
 * its updates, and renders whatever it currently has.
 */
const HyperMarkdown = forwardRef<HyperMarkdownHandle, HyperMarkdownProps>(
  function HyperMarkdown(props, ref) {
    const { md, streaming, animation, scrollDown } = props;
    ...
    if (streamRef.current === null) {
      streamRef.current = new MarkdownStream({
        md,
        streaming,
        animation,
        scrollDown,
