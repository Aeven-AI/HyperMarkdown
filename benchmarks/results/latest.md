...
Two things to know when reading text chars: HyperMarkdown's line-number
gutter counts as text, so it reads high on code fixtures; and
markstream-react virtualises long documents, so it reads low where a fixture
has many blocks — on `prose-mixed` it renders 14 of 60 headings and 39 of
180 list items, about 21% of the document.
